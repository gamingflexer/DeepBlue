...
Password = ''
# print(df.head(5))
browser = webdriver.Chrome(executable_path="D:\\Softwares\\chromedriver.exe")
...
file_name = "linkData.xlsx"
sheet = "Sheet1"

# Search Aju Bhai
nameToSearch = 'wegt qrgrgre wegfrwefcew'
browser.find_element_by_xpath('//input[@placeholder="Search"]').send_keys(nameToSearch)
...
for h in range(1, 6):
    ...
    # Search Aju Bhai
    nameToSearch = 'Aju Bhai'
    browser.find_element_by_xpath('//input[@placeholder="Search"]').send_keys(nameToSearch)
    browser.find_element_by_xpath('//input[@placeholder="Search"]').send_keys(u'\ue007')

    html = browser.page_source
    soup1 = BeautifulSoup(html, "html.parser")
    if soup1.find_all("div", attrs={"class": "search-no-results__image-container"}):
        print("inside if")
        ...
        for j in header_experience:
            ...
            if "Experience" in Header_Experience:
                # print("true")
                experience = soup.findAll("li", {
                    "class": "pv-profile-section__card-item-v2 pv-profile-section pv-position-entity ember-view"})
                for experience1 in experience:
                    try:
                        ...
                        location = experience1.find("h4",
                                                    {"class": "pv-entity__location t-14 t-black--light t-normal block"})
                        Title = re.sub('\n+', ' ', title.text)
                        Company = re.sub('\n+', ' ', company.text)
                        Time_Period = re.sub('\n+', ' ', time_period.text)
                        Location = re.sub('\n+', ' ', location.text)
        ...
        for j in header_education:
            ...
            if "Education" in Header_Education:
                education = soup.findAll("li", {
                    "class": "pv-profile-section__sortable-item pv-profile-section__section-info-item relative pv-profile-section__sortable-item--v2 sortable-item ember-view"})
                ...
                for education1 in education:
                    try:
                        ...
                        education_time_period = education1.find("p", {
        ...
        for j in header_skill:
            ...
            if "Skills & Endorsements" in Header_Skill:
                skill = soup.findAll("li", {
                    "class": "pv-skill-category-entity__top-skill pv-skill-category-entity pb3 pt4 pv-skill-endorsedSkill-entity relative ember-view"})
                for skill1 in skill:
                    try:
        ...
        for j in header_accomplishments:
            ...
            if "Accomplishments" in Header_Accomplishments:
                ...
                for accomplishments1 in accomplishments:
                    try:
                        accomplishments_title = accomplishments1.find("div", {
                            "class": "pv-accomplishments-block__list-container"})
        ...
        for j in header_interests:
            ...
            if "Interests" in Header_Interests:
                ...
                for interests1 in interests:
                    try:
                        interests_title = interests1.find("h3",
                                                          {"class": "pv-entity__summary-title t-16 t-black t-bold"})
